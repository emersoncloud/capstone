...
//connection or edge between two elements of the graph.
struct Node
{
    int dest;
	double weight;
...
struct graphTemplate {
		...
		//Pure virtual function that is intented to be overwritten by particular
		//graph implementation. 
		virtual double traverse(int src, int destination) = 0;
...
struct Resistive : graphTemplate
{
		
		using graphTemplate::graphTemplate;
		...
		//destination. 
		double traverse(int src, int destination) {
